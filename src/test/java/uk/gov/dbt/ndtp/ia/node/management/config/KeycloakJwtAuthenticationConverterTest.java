...
class KeycloakJwtAuthenticationConverterTest {
    ...
    private void setupMockIntrospectionResponse(Map<String, Object> response) {
        ...
        Mockito.lenient().when(restTemplate.postForEntity(anyString(), any(HttpEntity.class), Mockito.eq(JwtToken.class)))
    ...
    void convert_shouldExtractCorrectAuthorities() {
        ...
        List<String> expectedAuthorities = Arrays.asList(
            "ROLE_F1:TOPIC_1",
            "ROLE_F1:TOPIC_2",
            "ROLE_F2:R1",
            "ROLE_management-node:MyRole"
        );
    ...
    void convert_withNullResourceAccess_shouldNotFail() {
        ...
        EnhancedPrincipal principal = ((CustomJwtAuthenticationToken) token).getPrincipal();
        assertNotNull(principal);
        assertEquals("test-subject", principal.getSubject());
        assertEquals("unknown", principal.getClientId()); // Should default to "unknown"
