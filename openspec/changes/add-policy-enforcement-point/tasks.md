...
- [ ] 2.1 Add `PolicyDecisionRequest`/`PolicyDecisionResponse` DTOs (clientId, organisation, resource, action / result) under `service/providers/policy`; verify with a serialization unit test (Jackson round-trip matches OPA's `{"input": {...}}` / `{"result": ...}` shape from design.md - Decision 3).
